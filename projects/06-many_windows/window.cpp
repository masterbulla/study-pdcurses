...
namespace curses {
	...
	int Window::mvderwin(int x, int y) {
		int r = ::mvderwin(_impl->wnd, y, x);
		CURSES_ASSERT(r == OK);
		return r;
	}

	int Window::mvwin(int x, int y) {
		CURSES_ASSERT(!_impl->sub);// サブウィンドウの絶対座標移動は避けるべき
		return ::mvwin(_impl->wnd, y, x);
	}
	...
	Window::Ptr Window::subwin(int x, int y, int width, int height) {
		Window::Ptr p = std::make_shared<Window>();
		p->_impl->wnd = ::subwin(_impl->wnd, height, width, y, x);
		p->_impl->sub = true;
		return p;
	}

	Window::Ptr Window::derwin(int x, int y, int width, int height) {
		Window::Ptr p = std::make_shared<Window>();
		p->_impl->wnd = ::derwin(_impl->wnd, height, width, y, x);
		p->_impl->sub = true;
		return p;
	}
